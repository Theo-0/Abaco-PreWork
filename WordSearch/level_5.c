...
// LEVEL 5 (No level 4 for some reason)
// New Features
//   - words can be found orthogonally (with given simplification of 3-letters &
//     downwards) so doesn't use loops so method slightly different
...
// enum OrthogonalDirection { DOWN_RIGHT, RIGHT_DOWN, DOWN_LEFT, LEFT_DOWN };
enum { NOT_FOUND, DOWN_RIGHT, RIGHT_DOWN, DOWN_LEFT, LEFT_DOWN };
						  ...
						  FILE* fptr )
{
	...
	for (int i = 0; i < NUM_OF_KEYS; i++)
	{
		...
		int orthogonal_found = 0; // (1,2,3,4 -> pattern A - D, 0 -> not found)
		...
		for (int j = 0; j < MAX_GRID_BREADTH; j++)
		{
			for (int k = 0; k < MAX_GRID_LENGTH; k++)
			{
				...
				if (grid[j][k] == search_keys[i][0])
				{
					...
					// -------------------- Orthogonal --------------------
					// To simplify, it's given that these words are 3-letters
					// long & only occur downwards (so only 4 possible patterns)
					
					// 3rd letter is down right AND within bounds
					if ((grid[j + 1][k + 1] == search_keys[i][2]) &&
						((j + 1) <= MAX_GRID_BREADTH) &&
						((k + 1) <= MAX_GRID_LENGTH))
					{
						// 2nd letter is down OR right
						if (grid[j + 1][k] == search_keys[i][1])
							orthogonal_found = DOWN_RIGHT;
						else if (grid[j][k + 1] == search_keys[i][1])
							orthogonal_found = RIGHT_DOWN;
					}
					// 3rd letter is down left  AND within bounds
					else if ((grid[j + 1][k - 1] == search_keys[i][2]) &&
							 ((j + 1) <= MAX_GRID_BREADTH) &&
							 ((k - 1) >= 0))
					{
						// 2rd letter is down OR left
						if (grid[j + 1][k] == search_keys[i][1])
							orthogonal_found = DOWN_LEFT;
						else if (grid[j][k - 1] == search_keys[i][1])
							orthogonal_found = LEFT_DOWN;
					}
					else orthogonal_found = NOT_FOUND;

					// If word was found (orthogonally)
					if (orthogonal_found != NOT_FOUND)
						break;
			...
			if ((horizontal_found != 0) || (vertical_found != 0) ||
				(diagonal_found != 0) || (orthogonal_found != NOT_FOUND))
				break;
		...
		else if (orthogonal_found != NOT_FOUND)
			printf("\"%s\", Found, orthogonally @ row %d, column %d\n", search_keys[i], row_index + 1, column_index + 1);


		// if one of the directions has been found, update output
		// i.e. replace the position of the words chars with dots
		...
		else if ((orthogonal_found == DOWN_RIGHT))
		{
			output[row_index][column_index] = '.';
			output[row_index + 1][column_index] = '.';
			output[row_index + 1][column_index + 1] = '.';
		}
		else if (diagonal_found == RIGHT_DOWN)
		{
			output[row_index][column_index] = '.';
			output[row_index][column_index + 1] = '.';
			output[row_index + 1][column_index + 1] = '.';
		}
		else if ((orthogonal_found == DOWN_LEFT))
		{
			output[row_index][column_index] = '.';
			output[row_index + 1][column_index] = '.';
			output[row_index + 1][column_index - 1] = '.';
		}
		else if (diagonal_found == LEFT_DOWN)
		{
			output[row_index][column_index] = '.';
			output[row_index][column_index - 1] = '.';
			output[row_index + 1][column_index - 1] = '.';
